...
class UnitController {
  async createUnit(req, res) {
    const { error, value } = UnitValidator.createUnitSchema.validate(req.body, {
      abortEarly: false,
    });
    if (error) {
      return res
  ...
  async updateUnit(req, res) {
    const { error, value } = UnitValidator.updateUnitSchema.validate(req.body, {
      abortEarly: false,
    });
    if (error) {
      return res
  ...
  async getTenantUnits(req, res) {
    ...
    // Get tenant units
    const result = await UnitService.getTenantUnitsWithDetails(
      tenantId,
      filters
    );
    ...
    return res.status(200).json({
      success: true,
      message: "Tenant units retrieved successfully",
      data: {
        result,
      },
    });
